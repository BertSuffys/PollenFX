class ParticleSizeByLifeBehavior extends ParticleBehavior {
  ...
  withNoise(scalarNoise, uniformNoise = false) {
  ...
  act(particle, actTime, deltaTime, deltaTimeSeconds) {
    ...
    const xFullRangeProgress = (actTime / this.duration) * Math.max(1, xSteps - 1);
    ...
    const yFullRangeProgress = (actTime / this.duration) * Math.max(1, ySteps - 1);
